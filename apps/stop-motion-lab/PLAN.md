...
A one-user lab that turns a subject prompt (or a reference still) plus a motion prompt into a short looping stop-motion GIF via chained GPT-Image 2.5 Flare generate → edit hops — locally via Vite middleware and on the public Pages site via Cloudflare Pages Functions.
...
**Validation fallback:** when the key is missing (or `STUB_OPENAI=1`), the same handler returns canned stub PNGs so the UI does not crash. Production without a Pages secret stays in stub mode; with `OPENAI_API_KEY` set in Pages, live Flare generate/edit.

## Tasks

1. Scaffold Vite + React + TypeScript with Bun, `bunfig.toml`, and shadcn/ui. Set Vite `base: '/stop-motion-lab/'`.
2. Shared Bun/Vite API: `POST /api/generate` and `POST /api/edit` hold the key; clamp frame counts; Flare + `quality: low` + `1024x1024` PNG. Stub frames when no key.
...
4. Tests for frame-cap clamp, edit-prompt builder, and the JSON API contract only; no git hooks.
